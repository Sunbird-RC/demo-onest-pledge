...
public class MobileNumberAuthenticator extends AbstractUsernameFormAuthenticator implements Authenticator {
	...
	public void action(AuthenticationFlowContext context) {
		...
		if (type.equals(LOGIN_FORM)) {
			...
			System.out.println("Validation" +  iValidation.validate(mobileNumber));
			if(iValidation.validate(mobileNumber) || EMAIL_OTP ) {
	...
	private void callNotify(UserModel user, String otp, String key, String prefix) {
		...
		System.out.println( "Checking for mobile Nodes ----> " + mobileNumberNodes);
		if (mobileNumberNodes.size() > 0) {
			try {
				// System.out.println( "Params that are sent to notify ----> ", prefix, " ", mobileNumberNodes.get(0), " otp ", otp);
